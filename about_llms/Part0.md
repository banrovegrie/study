...
Language modeling is fundamentally a **sequence prediction** problem. Given a sequence of tokens `(x₁, x₂, ..., x_{t-1})`, we aim to model the probability distribution over the next token:
...
Where:
- `xₜ ∈ ℝᵈ` is the input embedding at time t
- `hₜ ∈ ℝⁿ` is the hidden state
- `W_hh ∈ ℝⁿˣⁿ` is the recurrent weight matrix
- `W_xh ∈ ℝⁿˣᵈ` is the input weight matrix
- `tanh` provides nonlinearity and bounds outputs to [-1, 1]
...
```
∂L/∂h₁ = ∂L/∂hₜ · ∏(t=2 to T) ∂hₜ/∂h_{t-1}
```

Each Jacobian term is:

```
∂hₜ/∂h_{t-1} = diag(tanh'(zₜ)) · W_hh
```

Where `tanh'(z) = 1 - tanh²(z) ≤ 1`.
...
```
‖∏(t=2 to T) ∂hₜ/∂h_{t-1}‖ ≤ ∏(t=2 to T) ‖∂hₜ/∂h_{t-1}‖ ≈ γ^(T-1)
```

Where `γ = ‖diag(tanh') · W_hh‖`.

- If `γ < 1`: gradients **vanish** exponentially as T grows
- If `γ > 1`: gradients **explode** exponentially
...
Instead of transforming the hidden state through a nonlinearity at each step, LSTMs maintain a **cell state** `Cₜ` that can flow unchanged through time:

```
        Cell State C_{t-1} ─────────────────────────────────► Cₜ
                              │           │           │
                            [×]─────────[+]─────────[×]
                              ↑           ↑           ↓
                           Forget       Input      Output
...
Where:
- `σ` is the sigmoid function: `σ(x) = 1/(1 + e^(-x)) ∈ (0, 1)`
...
When `fₜ ≈ 1` (forget gate open), gradients flow **unchanged**:

```
∂L/∂C₁ = ∂L/∂Cₜ · ∏(t=2 to T) fₜ ≈ ∂L/∂Cₜ
```

The network **learns** when to remember (f ≈ 1) vs. forget (f ≈ 0).
...
h̃ₜ = tanh(W · [rₜ ⊙ h_{t-1}, xₜ])    # candidate
hₜ = (1 - zₜ) ⊙ h_{t-1} + zₜ ⊙ h̃ₜ    # interpolation
```
...
|------------|-------------|--------|
| **Sequential computation** | Must process xₜ before x_{t+1} | Cannot parallelize on GPU |
| **Fixed-size bottleneck** | All history compressed into hₜ ∈ ℝⁿ | Information loss inevitable |
| **O(T) path length** | Information from x₁ must flow through T-1 steps to reach xₜ | Signal degradation |
...
- `σ(xV + c)`: the "gate" pathway (how much to let through)
...
Where `Swish(x) = x · σ(βx)` (typically β = 1).
...
To match parameter count, `d_ff` is reduced by factor of 2/3:

```
d_ff^{SwiGLU} = (2/3) × d_ff^{standard}
```
...
Given:
- Query `q ∈ ℝ^{d_k}`: what we're looking for
- Keys `K ∈ ℝ^{n × d_k}`: what each position contains
- Values `V ∈ ℝ^{n × d_v}`: information at each position
...
The `√d_k` scaling prevents scores from becoming too large (which would make softmax saturate).
...
```
αᵢ = softmax(scores)ᵢ = exp(score(q, kᵢ)) / Σⱼ exp(score(q, kⱼ))
```
...
```
context = Σᵢ αᵢvᵢ = softmax(qKᵀ / √d_k) V
```

#### 5.3.4 Compact Matrix Form

For a batch of queries `Q ∈ ℝ^{m × d_k}`:
...
|-----------|-----------------|------------------|
| Computing QKᵀ | O(n²d) | O(n²) |
| Softmax | O(n²) | O(n²) |
| Multiplication with V | O(n²d) | O(nd) |
| **Total** | **O(n²d)** | **O(n²)** |
...
Different dimensions encode position at different frequencies (wavelengths from 2π to 10000·2π).
...
For a query/key vector, split into pairs of dimensions `(q_{2i}, q_{2i+1})` and apply rotation by angle `θᵢ · m` where m is the position:

```
┌─────────┐   ┌──────────────────────────────┐   ┌─────────┐
│ q'_{2i} │   │ cos(m·θᵢ)   -sin(m·θᵢ)       │   │ q_{2i}  │
│         │ = │                              │ × │         │
│q'_{2i+1}│   │ sin(m·θᵢ)    cos(m·θᵢ)       │   │q_{2i+1} │
└─────────┘   └──────────────────────────────┘   └─────────┘
```

Where `θᵢ = 10000^(-2i/d)`.

**Why this works**: When computing dot product `q · k`, the rotation angles **subtract**, giving **relative position**:
...
Where `μ = (1/d)Σᵢ xᵢ` and `σ = √((1/d)Σᵢ (xᵢ - μ)²)`.
...
```
KV cache = 2 × layers × seq_len × d_model × 2 bytes
```
...
Train reward model R_φ to predict preferences:
```
L_RM = -log σ(R_φ(A) - R_φ(B))
```
...
```
L_PPO = E[min(rₜ(θ) Aₜ, clip(rₜ(θ), 1-ε, 1+ε) Aₜ)] - β D_KL(π_θ ‖ π_ref)
```

Where:
- `rₜ(θ) = π_θ(aₜ|sₜ) / π_old(aₜ|sₜ)` is the probability ratio
- `Aₜ` is the advantage (how much better than baseline)
...
Where `W^{DKV} ∈ ℝ^{d × d_c}` and `d_c ≪ d · n_h`.
...
│        │                   │                                 │             │
│        ▼                   ▼                                 ▼             │
│       qₜ              c^{KV}_t   ← Cache this!           k^R_t (RoPE)      │
│        │                   │                                 │             │
│        │           ┌───────┴───────┐                         │             │
│        │           ▼               ▼                         │             │
│        │      ┌────────┐     ┌────────┐                      │             │
...
│   │   Attention(qₜ, K, V) = softmax(qKᵀ/√d)V │                             │
│   └──────────────────────────────────────────┘                             │
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
```
...
```
L_MTP = -Σₖ λₖ log P(x_{t+k} | x_{≤t}, x̂_{t+1}, ..., x̂_{t+k-1})
```
...
MTP Module:
    Input: hₜ, embedding(predicted_{t+k-1})
...
Standard attention: O(L²) for sequence length L

For 128K context: 128000² = 16.4 billion attention computations per layer!
...
│    │                                                          │            │
│    │    output = softmax(q · K_sparseᵀ / √d) · V_sparse       │            │
│    └──────────────────────────────────────────────────────────┘            │
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
```
...
```
reward = outcome_reward + length_penalty + language_consistency
```

- Penalize unnecessarily long responses
- Encourage consistent language (no mixing)
...
Transformers scale as O(L²) in sequence length. For very long sequences, this becomes prohibitive.
...
Where:
- `x(t) ∈ ℝ` is the input signal
- `h(t) ∈ ℝᴺ` is the hidden state
- `y(t) ∈ ℝ` is the output
- `A ∈ ℝᴺˣᴺ` is the state matrix

#### 11.2.2 Discretization

For sequence modeling, discretize with step size Δ:
...
```
wkvₜ = (Σᵢ₌₁ᵗ⁻¹ exp(-(t-1-i)w + kᵢ) vᵢ + exp(u + kₜ) vₜ) /
       (Σᵢ₌₁ᵗ⁻¹ exp(-(t-1-i)w + kᵢ) + exp(u + kₜ))
```

Key features:
- Exponential decay `exp(-(t-i)w)` for past tokens
...
```
q(xₜ | x₀) = mask x₀ with probability βₜ
```

**Reverse process** (denoise by predicting masks):
```
p_θ(x_{t-1} | xₜ) = predict masked tokens
```
...
- Outer: Update parameters θ
- Inner: The momentum vₜ is itself learning to compress gradient history
...
Where f_φ is a small neural network that learns **how to accumulate gradients**.
...
Where `Σ = diag(σ₁, σ₂, ..., σᵣ)` contains singular values.

**SVF**: Learn task-specific **scaling factors** for singular values:
```
Σ_task = diag(s₁σ₁, s₂σ₂, ..., sᵣσᵣ)
W_task = U Σ_task Vᵀ
```

**Benefits:**
- Very few parameters per task (just the scalars sᵢ)
